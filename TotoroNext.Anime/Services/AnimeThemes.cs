...
internal class AnimeThemes(IHttpClientFactory httpClientFactory) : IAnimeThemes
{
    public async Task<List<Abstractions.AnimeTheme>> FindById(long id, string serviceName)
    {
        try
        {
            ...
            var searchResponse = await client.GetFromJsonAsync<AnimeThemesSearchResponse>($"https://api.animethemes.moe/anime?filter[has]=resources&filter[site]={serviceName}&filter[external_id]={id}");
            ...
            var animeResponse = await client.GetFromJsonAsync<AnimeThemesResponse>($"https://api.animethemes.moe/anime/{slug}?include=animethemes.animethemeentries.videos.audio,animethemes.song.artists");
            return animeResponse?.Anime.AnimeThemes.Select(x => new Abstractions.AnimeTheme
            {
                ...
                Artist = string.Join(",", x.Song.Artists.Select(x => x.Name))
            }).ToList() ?? [];
        }
        catch
        {
...
internal class AnimeThemesSearchResponse
{
    ...
    public List<AnimeItem> Anime { get; set; } = [];
}

internal class AnimeThemesResponse
{
    [JsonPropertyName("anime")]
    public AnimeItem Anime { get; set; } = new();
}
...
internal class AnimeTheme
{
    ...
    public AnimeSong Song { get; set; } = new();
...
internal class AnimeSong
{
    ...
    public List<AnimeArtist> Artists { get; set; } = [];
}
...
internal class AnimeThemeEntry
{
    ...
    public List<AnimeThemeVideo> Videos { get; set; } = [];
}
...
internal class AnimeThemeVideo
{
    ...
    public AnimeAudio Audio { get; set; } = new();
}
